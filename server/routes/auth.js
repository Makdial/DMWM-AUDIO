...
router.get("/getUsers", async (req, res) => {
    const options = {
      ...
      if(cursor){
        ...
        res.status(400).send({success : false, msg : "Pas de donnees trouvees"});
      }
});
...
router.delete("/deleteUser/:userId", async (req, res) => {
    ...
    if(result.deleteCount === 1) {
        res.status(200).send({success : true, msg: "Client Suprrimer"})
    }else{
        res.status(500).send({success : false, msg: "Client N'es pas trouve(e)"})
    }
})
